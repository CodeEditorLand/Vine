...
/// Handles : `outputChannel.replace` Atomic buffer replacement: equivalent to `clear` + `append`, but rendered as a single workbench frame so the user does not see an empty flash..
