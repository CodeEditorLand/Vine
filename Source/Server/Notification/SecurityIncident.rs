...
/// Handles : `security.incident` notification: Cocoon-side security policy
/// flagged a breach (extension violated permission set, blocked filesystem
/// access, etc.). Land has no central security dashboard yet; the atom surfaces
/// the incident via `dev_log!` and re-emits on `sky://security/incident` for
/// future listeners..
