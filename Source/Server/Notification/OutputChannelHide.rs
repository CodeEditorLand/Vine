...
/// Handles : `outputChannel.hide` Forwards to Sky as `sky://output/show {
/// visible: false, channel }` so the workbench panel can dismiss the channel
/// via the same handler that processes `show()` calls..
