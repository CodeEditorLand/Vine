...
/// Handles : → `languages.setDocumentLanguage` Emitted when an extension calls `vscode.languages.setTextDocumentLanguage`. Forwarded verbatim to Sky on `sky://languages/setDocumentLanguage` so Monaco swaps the language mode on the matching editor..
