...
/// Handles : `tree.refresh` notification: extension's
/// `TreeDataProvider.onDidChangeTreeData` emitter fired. Relayed to Sky as
/// `sky://tree-view/refresh`; Sky resolves the view via
/// `TreeViewByViewId(viewId)` and calls `ITreeView.refresh()`..
