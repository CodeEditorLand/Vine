...
//!
//! Synthesised from `Mountain/Source/Vine/Client/` per
//! `.hermes/plan/Vine-Synthesis-Audit.md`. The atomization layout matches
//! Mountain's exactly - one entry-point per file:
//!
...
//!   `crate::DefaultRequestTimeoutMs`; per-call override) - `SendRequest`.
//! - Atomic-counter request IDs (`AtomicU64`, no `SystemTime` syscall) -
//!   `SendRequest`.
//! - h2 transport tuning (4 MB stream / 16 MB connection windows,
//!   `LAND_TONIC_TUNED=0` to disable) - `TryConnectSingle`.
